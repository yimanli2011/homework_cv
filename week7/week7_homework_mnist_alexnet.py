...
def print_f(msg):
    print(msg)
    pass
# 完成对一个层的flops的计算：（只算了conv2d层？）
def get_flops(layer,fea):
    #import pdb
    #pdb.set_trace()
    # 这里填入flops计算方法：(1次加、乘算一个flops)
    flops = (layer.in_channels * layer.kernel_size[0]**2) * fea.shape[2]**2 *layer.out_channels
    flops = float(flops)
    print_f(layer)
...
def print_params_num(layers):
    ...
    for i in [0,2,4,5,6,8,9,10]: 
        ...
        if i == 8 or i==9 or i==10:
            layer_num = layers[i].in_features * layers[i].out_features + 1
        else:
            layer_num = (layers[i].in_channels * layers[i].kernel_size[0]**2 +1) * layers[i].out_channels
        # 换算为k
...
if __name__=="__main__":
    ...
    # add conv1 
    # 填写输入，输出通道数
    conv1=nn.Conv2d(3,96,kernel_size = 11,stride=4,padding=2) #(224+4-11)/4+1=55
    layers.append(conv1)
    # 填写kernel_size 和stride
    pool2=nn.MaxPool2d(kernel_size= 3, stride= 2, padding=0,ceil_mode=True) #(55-3)/2+1=27  ceil_mode 天花板模式
    layers.append(pool2)
    # add conv3 
    # 填写输入，输出通道数
    conv3=nn.Conv2d(96,256,kernel_size = 5,stride=1,padding=2) #(27+4-5)/1+1=27
    layers.append(conv3)
    pool4=nn.MaxPool2d(kernel_size=3, stride=2, padding=0) #(27-3)/2+1=13
    layers.append(pool4)
    # add conv5 
    # 填写输入，输出通道数
    conv5=nn.Conv2d(256,384,kernel_size = 3,stride=1,padding=1) #(13+2-3)/1+1=13
    layers.append(conv5)
    # 填写输入，输出通道数
    conv6=nn.Conv2d(384,384,kernel_size = 3,stride=1,padding=1) #(13+2-3)/1+1=13
    layers.append(conv6)
    # 填写输入，输出通道数
    conv7=nn.Conv2d(384,256,kernel_size = 3,stride=1,padding=1) #(13+2-3)/1+1=13
    layers.append(conv7)
    # 填写kernel_size 和stride
    pool8=nn.MaxPool2d(kernel_size=3, stride=2, padding=0) #(13-3)/2+1=6
    layers.append(pool8)
    # 填写输入，输出神经元数
    fc9 = nn.Linear(256*6*6,4096)
    layers.append(fc9)
